...
export const LandingPage: React.FC<LandingPageProps> = ({ onGetStarted }) => {
  ...
  const getHighlightClass = (index: number) => {
    if (highlightIndex === 3) return "text-white opacity-100"; // Show all
    if (highlightIndex === index) return "text-white opacity-100";
    // Increased opacity from 0.3 to 0.5 for accessibility contrast (slate-500)
    return "text-slate-500 opacity-50"; 
  };

  const getStepClass = (index: number) => {
    // State 4 is "Show All"
    if (howItWorksIndex === 4) return "opacity-100 scale-100 bg-slate-800/80 border-slate-700";
    
    // Active State
    if (howItWorksIndex === index) return "opacity-100 scale-105 bg-slate-800 border-blue-500/50 shadow-lg shadow-blue-900/10";
    
    // Inactive State
    return "opacity-40 scale-95 bg-transparent border-transparent";
  };
